...
import { Activity, ArrowRight, BarChart3, CalendarClock, Clock3, Download, FileWarning, ListChecks, Search } from "lucide-react";
...
import type { ReportActivityItem, ReportRange, ReportTimelinePoint, ReportWorkloadItem } from "@/lib/types";
...
const workloadColumns: DataColumn<ReportWorkloadItem>[] = [
  { id: "member", header: "Team member", primary: true, className: "workload-col-member", sortValue: (item) => item.display_name, cell: (item) => <IdentityCell displayName={item.display_name} email={item.email} /> },
  { id: "role", header: "Role", className: "workload-col-role", sortValue: (item) => item.role, cell: (item) => titleCase(item.role) },
  { id: "active", header: "Active", numeric: true, className: "workload-col-number", sortValue: (item) => item.active_tasks, cell: (item) => item.active_tasks },
  { id: "overdue", header: "Overdue", numeric: true, className: "workload-col-number", sortValue: (item) => item.overdue_tasks, cell: (item) => <span className={item.overdue_tasks ? "attention" : ""}>{item.overdue_tasks}</span> },
  { id: "completed", header: "Completed", numeric: true, className: "workload-col-number", sortValue: (item) => item.completed_in_period, cell: (item) => item.completed_in_period },
];
...
function chartStepFor(maxValue: number) {
  const roughStep = Math.max(maxValue, 5) / 5;
...
export default function ReportsPage() {
  ...
  const chartPeak = useMemo(() => Math.max(0, ...(report?.timeline.map(timelineTotal) ?? [0])), [report]);
  const chartStep = chartStepFor(chartPeak);
  const chartMax = chartStep * 5;
  const chartTicks = Array.from({ length: 6 }, (_, index) => chartMax - (chartStep * index));
  ...
  const workload = useMemo(() => {
    const search = workloadSearch.trim().toLowerCase();
    return (report?.workload ?? []).filter((item) => !search || `${item.display_name} ${item.email} ${item.role}`.toLowerCase().includes(search));
  }, [report?.workload, workloadSearch]);

  return <div className="page reports-page">
    <div className="page-heading reports-heading"><div><h1 className="page-title">Reports</h1><p className="page-description">Contract processing, evidence coverage, human execution, and reviewer workload for the selected period.</p></div>{report && hasData && !isDemo ? <button className="button secondary" type="button" onClick={() => void api.downloadReportExport(organizationId, range)}><Download size={16} />Export CSV</button> : <button className="button secondary" disabled title={isDemo ? "Exports are disabled in the read-only demo." : "No report rows are available for export."}><Download size={16} />Export CSV</button>}</div>
    ...
    {query.isLoading ? <PageLoading rows={9} /> : query.error ? <PageError error={query.error} /> : report && (!hasData ? <section className="inline-state report-empty"><BarChart3 size={20} /><div><h2>No reportable activity for this period</h2><p>Choose another period or create a contract review in an authorised workspace.</p></div><div>{canUpload && <Link className="button" href="/contracts/new">Upload a contract <ArrowRight size={15} /></Link>}{isDemo && <Link className="button secondary" href="/contracts/demo-msa">Explore sample report</Link>}</div></section> : <>
      ...
      <section className="report-flow-section">
        <div className="report-chart-card">
          <div className="report-chart-header">
            <div><h2>Workspace activity</h2><div className="report-chart-metric"><strong>{chartTotal}</strong><span>events</span><span className="report-chart-selection" aria-live="polite">{selected ? timelineTotal(selected) : 0}</span><small>{selected ? `in ${selected.label}` : "in the selected range"}</small></div></div>
            <div className="report-chart-legend" aria-label="Chart legend"><span className="contracts">Contracts</span><span className="tasks">Tasks created</span><span className="completed">Tasks completed</span><span className="decisions">Human decisions</span></div>
          </div>
          <div className="report-chart-canvas" style={{ "--period-count": report.timeline.length } as React.CSSProperties}>
            <div className="report-chart-axis" aria-hidden="true">{chartTicks.map((tick) => <span key={tick}>{tick}</span>)}</div>
            <div className="report-chart-scroll">
              <div className="report-chart-plot">
                <div className="report-chart-gridlines" aria-hidden="true">{chartTicks.map((tick) => <i key={tick} />)}</div>
                <div className="report-bars" role="group" aria-label="Workspace activity by period">{report.timeline.map((point) => {
                  ...
                  return <div className={`report-bar-period${focusedPeriod === point.period_start ? " is-active" : ""}`} key={point.period_start} role="img" tabIndex={0} aria-label={`${point.label}: ${point.contracts_created} contracts, ${point.tasks_created} tasks created, ${point.tasks_completed} tasks completed, ${point.decisions_recorded} decisions`} onMouseEnter={() => setFocusedPeriod(point.period_start)} onMouseLeave={() => setFocusedPeriod(null)} onFocus={() => setFocusedPeriod(point.period_start)} onBlur={() => setFocusedPeriod(null)}><div className="report-bar-track"><div className="report-bar-stack" style={{ height: `${total ? (total / chartMax) * 100 : 0}%` }}>{point.decisions_recorded > 0 && <i className="decisions" style={{ flex: point.decisions_recorded }} />}{point.tasks_completed > 0 && <i className="completed" style={{ flex: point.tasks_completed }} />}{point.tasks_created > 0 && <i className="tasks" style={{ flex: point.tasks_created }} />}{point.contracts_created > 0 && <i className="contracts" style={{ flex: point.contracts_created }} />}</div></div><span>{point.label}</span></div>;
                })}</div>
              </div>
            </div>
          </div>
      ...
      <section className="report-governance-grid"><TableCard className="report-workload" title="Reviewer workload" description="Assigned work and completions in the selected period" actions={<label className="table-card-search"><Search size={16} /><input value={workloadSearch} onChange={(event) => setWorkloadSearch(event.target.value)} placeholder="Search reviewers" aria-label="Search reviewer workload" /></label>}><DataTable ariaLabel="Reviewer workload" columns={workloadColumns} rows={workload} rowKey={(item) => item.user_id} empty={<p className="table-empty-row">No reviewers match this search.</p>} pageSize={5} showFooter /></TableCard><aside className="report-activity"><div className="section-heading"><div><h2>Recent activity</h2><p>Append-only workspace history</p></div><Activity size={17} /></div><div>{report.recent_activity.length ? report.recent_activity.slice(0, 8).map((item) => <article key={item.id}><span /><div><strong>{activityLabels[item.action] ?? titleCase(item.action.replaceAll(".", " "))}</strong><p>{activityContext(item)} · {item.actor_name}</p></div><time dateTime={item.created_at}>{formatDate(item.created_at, { day: "numeric", month: "short" })}</time></article>) : <p className="report-no-activity">No activity falls within this period.</p>}</div></aside></section>
