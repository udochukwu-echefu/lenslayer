...
export default function InboxPage() {
  ...
  return <div className="page"><div className="page-heading"><div><h1 className="page-title">Inbox</h1><p className="page-description">Reviews, overdue actions, processing problems, and identity cases that need attention.</p></div></div>
    {(query.isLoading || taskQuery.isLoading || verificationQuery.isLoading) ? <PageLoading rows={7} /> : query.error ? <PageError error={query.error} /> : taskQuery.error ? <PageError error={taskQuery.error} /> : verificationQuery.error ? <PageError error={verificationQuery.error} /> : (!work.length && !overdue.length && !dueSoon.length && !pendingCases.length) ? <EmptyContracts compact message="No reviews need your attention." /> : <div className="inbox-sections">
      ...
      {pendingCases.length > 0 && <section><div className="section-heading"><h2>Identity verification <span className="count">{pendingCases.length}</span></h2><p><ShieldCheck size={13} />Separate governance module</p></div><div className="inbox-verification-list">{pendingCases.map((item) => <Link href={`/verify/${item.id}`} key={item.id}><span className={`verify-score ${item.risk_score >= 70 ? "high" : item.risk_score >= 20 ? "medium" : "low"}`}><strong>{item.risk_score}</strong><small>/100</small></span><div><strong>{item.applicant_name}</strong><p>{item.reference} · {item.finding_count} discrepancies · Synthetic</p></div><span className={`pill ${verificationTone(item.suggested_action)}`}>{item.suggested_action}</span><ArrowRight size={15} /></Link>)}</div></section>}
