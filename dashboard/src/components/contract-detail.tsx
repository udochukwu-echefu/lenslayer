...
export function ContractDetail({ contractId }: { contractId: string }) {
  const { activeOrganization, canDelete, canUpload, isDemo } = useWorkspace();
  ...
  const reviewQuery = useQuery({ queryKey: ["review", organizationId, contractId], queryFn: () => api.review(organizationId, contractId), enabled: ["ready", "reviewed"].includes(contractQuery.data?.status ?? ""), retry: (count, error) => !(error instanceof ApiError && error.status === 404) && count < 1 });
