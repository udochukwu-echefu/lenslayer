...
export function ContractCollaboration({ contractId }: { contractId: string }) {
  const { activeOrganization, canUpload, canManageTeam, user, isDemo } = useWorkspace();
  ...
  return <section className="collaboration-workspace">
    ...
    <div className="collaboration-columns">
      ...
      <aside className="collaboration-side">
        {!isDemo && <section><p className="eyebrow">Professional handoff</p><h3>Prepare counsel</h3><p>Download a DOCX package with evidence, unresolved risks, decisions, actions, and approvals.</p><button className="button secondary" type="button" onClick={() => void api.downloadCounselHandoff(organizationId, contractId)}><Download size={14} />Counsel handoff</button></section>}
