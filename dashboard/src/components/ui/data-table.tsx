...
export function DataTable<T>({
  ...
  return <div className="data-table-region">
    ...
    <div className="data-mobile-list" aria-label={ariaLabel}>{visible.map((row) => <article key={rowKey(row)}>{columns.map((column) => <div key={column.id} className={`${column.primary ? "mobile-primary" : ""} ${column.className ?? ""}`.trim()}>{!column.primary && <span>{column.mobileLabel ?? column.header}</span>}{column.primary && rowHref ? <Link href={rowHref(row)}>{column.cell(row)}</Link> : column.cell(row)}</div>)}</article>)}</div>
