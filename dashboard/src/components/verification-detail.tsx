...
export function VerificationDetail({ caseId }: { caseId: string }) {
  ...
  return <div className="page verification-detail">
    ...
    <header className="verification-case-header"><div><p className="case-reference">Identity verification · {item.reference}</p><h1>{item.applicant_name}</h1><p>Submitted {formatDate(item.submitted_at)} · {item.document_count} evidence source{item.document_count === 1 ? "" : "s"} · {titleCase(item.priority)} priority</p></div><div className="verification-recommendation"><span>Automated recommendation</span><strong className={verificationTone(item.suggested_action)}>{item.suggested_action}</strong><small>{latest ? `Latest human decision: ${latest.decision}` : item.assigned_to_name ? `Assigned to ${item.assigned_to_name}` : "Unassigned"}</small></div></header>
